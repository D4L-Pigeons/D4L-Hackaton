...
def combine_with_defaults(config, modalities_draft_config):
    ...
    for modalitiy_name, modality_config in modalities_draft_config.items():
        modality_dict = {f"{modalitiy_name}_{key}": value for key, value in modality_config.items()}
...
def _main_parser(parser):  # Check if needs to return parser
    ...
    parser.add_argument("--mode", type=str, choices=["train", "test"], default="train", help="Mode to run the model.")
    parser.add_argument(
...
def _data_parser(parser):
    ...
    parser.add_argument("--data_normalization", type=str, choices=["log1p", "standardize", "pearson_residuals", None], default="standardize", help="Normalization method for the data.")
...
def _train_parser(parser):
    ...
    parser.add_argument( # previously patience
        "--early-stopping-patience", default=2, type=int, help="Patience for EarlyStopping."
    )
...
def _modality_parser(parser, modalities_names):
    for modality_name in modalities_names:
        ...
        parser.add_argument(f"--{prefix}dim", type=int, default=13953 if modality_name == "gex" else 134, help=f"Dimension of the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}hidden_dim", type=int, default=128, help=f"Hidden dimension for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}encoder_hidden_dim", type=int, default=128, help=f"Encoder hidden dimension for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}encoder_out_dim", type=int, default=32, help=f"Encoder output dimension for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}latent_dim", type=int, default=16, help=f"Latent dimension for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}decoder_hidden_dim", type=int, default=128, help=f"Decoder hidden dimension for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}batch_norm", type=bool, default=False, help=f"Whether to use batch normalization for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}dropout_rate", type=float, default=0.1, help=f"Dropout rate for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}recon_loss_coef", type=float, default=1, help=f"Reconstruction loss coefficient for the {modality_name.upper()} modality.")
        parser.add_argument(f"--{prefix}kld_loss_coef", type=float, default=0.1, help=f"KLD loss coefficient for the {modality_name.upper()} modality.")
